    ...
    <div>
        ...
        <div class="row">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tbody>
                                    @forelse ($productCategory as $data)
                                        <tr>
                                            <th scope="row">{{ $data->id }}</th>
                                            <td><span class="text-15 font-weight-200">{{ $data->lib }}</span><br /></td>
                                            <td><span class="text-15 font-weight-200">
                                                @if ($data->commercialisable == 1)
                                                    Oui
                                                @else
                                                    Non
                                                @endif
                                            ...
                                            <td><button class="btn btn-outline-success m-1" type="button" data-toggle="modal" data-target="#verifyModalContent{{ $data->id }}" data-whatever="@fat">Modifier</button><a href="{{ route('stock.category_prod.destroy',['slug'=> $data->slug]) }}"><button class="btn btn-outline-danger m-1" type="button">Supprimer</button></a></td>
                                        </tr>
                                            ...
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="verifyModalContent{{ $data->id }}_title">Modification de la catégorie {{ $data->lin }}</h5>
                                                        <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <form method="put" action="{{ route('stock.category_prod.update', ['slug'=>$data->slug])}}">
                                                            @csrf
                                                            @method('PUT')
                                                            <div class="form-group">
                                                                <label class="col-form-label" for="recipient-name-3{{ $data->id }}">Libellé:</label>
                                                                <input class="form-control" value="{{ $data->lib }}" id="recipient-name-3{{ $data->id }}" type="text" name="lib"/>
                                                            </div>
                                                            <div class="form-group row">
                                                                <label class="switch pr-5 switch-success ml-3 mt-3"><span>Catégorie de produits commercialisable</span>
                                                                    <input type="checkbox" @if ($data->commercialisable == 1) checked @endif  name="commercialisable" /><span class="slider"></span>
                                                                </label>
                                                            </div>
            ...
            <div class="col-lg-5 col-xl-5 mb-4 mt-0">
                <form method="POST" action="{{ route('stock.category_prod.store')}}">
