...
@section('title', 'Details fournisseur -')
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="ul-product-detail__border-box">
                            ...
                            <p class="text-muted text-12">Mettre à jour l'image du fournisseur actuel.</p>
                        </div>
                    </div>
                </div>
            </a>
        </div>
        <div class="modal fade" id="add_image{{ $fournisseur->id }}" tabindex="-1" role="dialog" aria-labelledby="add_image{{ $fournisseur->id }}" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="add_image{{ $fournisseur->id }}_title">Modifier l'image du fournisseur</h5>
                        <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                    </div>
                    <div class="modal-body">
                        <form method="post" action="{{ route('stock.fournisseurs.add_image',['slug'=>$fournisseur->slug]) }}" enctype="multipart/form-data">
                            @csrf
                            @method('POST')
            ...
            <div class="card">
                <div class="card-body">
                    <div class="ul-product-detail__border-box">
                        ...
                        <p class="text-muted text-12">Afficher la liste de tous les fournisseurs en base.</p>
                    </div>
                </div>
            </div>
        </a>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="ul-product-detail__border-box">
                        ...
                        <p class="text-muted text-12">Effectuer un mouvement de stock pour ce fournisseur</p>
                    </div>
                </div>
            </div>
            </a>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="ul-product-detail__border-box">
                        ...
                        <h5 class="heading">Supprimer ce fournisseur</h5>
                        <p class="text-muted text-12">Le fournisseur disparaîtra définitivement de votre liste fournisseurs.</p>
                    </div>
                </div>
            </div>
            </a>
                    ...
                    <div class="row">
                        <div class="col-lg-4">
                            <div class="ul-product-detail__image">
                                @if ($fournisseur->image)
                                    <img class="w-100" src="{{ asset($fournisseur->image) }}" alt="alt" />
                                @else
                        ...
                        <div class="col-lg-8">
                            <div class="ul-product-detail__brand-name mb-4">
                                <h2 class="heading">{{ $fournisseur->name }}</h2><span class="text-warning">{{ $fournisseur->category->name }}</span>
                            </div>
                            @if ($fournisseur->price)
                                <div class="ul-product-detail__price-and-rating d-flex align-items-baseline">
                                    <h5 class="font-weight-700 text-primary mb-0 mr-2">{{ number_format($fournisseur->price , 0, '', ' ') }} Fcfa</h5>
                                    <span class="text-mute font-weight-800 mr-2">
                                </div>
                            @endif

                            <div class="ul-product-detail__features mt-4">
                                <h5 class="font-weight-700">Caractéristiques du fournisseur:</h5>
                                <ul class="m-0 p-0">
                                    <div class="ul-widget-app__browser-list-1 mb-2 mt-4">
                                        <i class="i-Spell-Check text-white teal-500 rounded-circle p-2 mr-3"></i>
                                        <span class="text-15"><strong>Référence :</strong> {{ $fournisseur->reference }} </span>
                                    ...
                                    {{-- <div class="ul-widget-app__browser-list-1 mb-2">
                                        ...
                                        <span class="text-15"><strong>Stock alerte (seuil) :</strong> {{ $fournisseur->stockalert }}</span>
                                        <span class="text-mute" style="display: none">2 April </span>
                                    </div> --}}
                                    <div class="ul-widget-app__browser-list-1 mb-2 ">
                                        <i class="i-Spell-Check text-white teal-500 rounded-circle p-2 mr-3"></i>
                                        <span class="text-15"><strong>Type :</strong> {{ $fournisseur->parametre('type')->name }}</span>
                                        <span class="text-mute" style="display: none">2 April </span>
                                    </div>
                                    <div class="ul-widget-app__browser-list-1 mb-2 ">
                                        <i class="i-Spell-Check text-white teal-500 rounded-circle p-2 mr-3"></i>
                                        <span class="text-15"><strong>Nature :</strong> {{ $fournisseur->parametre('nature')->name }}</span>
                                        <span class="text-mute" style="display: none">2 April </span>
                                    </div>
                                    <div class="ul-widget-app__browser-list-1 mb-2 ">
                                        <i class="i-Spell-Check text-white teal-500 rounded-circle p-2 mr-3"></i>
                                        <span class="text-15"><strong>Unité :</strong> {{ $fournisseur->parametre('unite')->name }}</span>
                                        <span class="text-mute" style="display: none">2 April </span>
                                    </div>
                                    {{-- @if ($fournisseur->poids && $fournisseur->unite_poids )
                                        <div class="ul-widget-app__browser-list-1 mb-2 "><i class="i-Spell-Check text-white teal-500 rounded-circle p-2 mr-3"></i><span class="text-15"><strong>Poids :</strong> {{ $fournisseur->poids }} {{ $fournisseur->unite_poids }}</span><span class="text-mute" style="display: none">2 April </span></div>
                                    @endif
                                    @if ($fournisseur->unite_mesure && $fournisseur->longueur)
                                        <div class="ul-widget-app__browser-list-1 mb-2 "><i class="i-Spell-Check text-white teal-500 rounded-circle p-2 mr-3"></i><span class="text-15"><strong>Dimensions :</strong> {{ $fournisseur->longueur }}x{{ $fournisseur->largeur }}x{{ $fournisseur->hauteur }} {{ $fournisseur->unite_mesure }}</span><span class="text-mute" style="display: none">2 April </span></div>
                                    @endif
                                    @if ($fournisseur->volume && $fournisseur->unite_volume)
                                        <div class="ul-widget-app__browser-list-1 mb-2 "><i class="i-Spell-Check text-white teal-500 rounded-circle p-2 mr-3"></i><span class="text-15"><strong>Volume :</strong> {{ $fournisseur->volume }} {{ $fournisseur->unite_volume }}</span><span class="text-mute" style="display: none">2 April </span></div>
                                    @endif
                                    @if ($fournisseur->liquide)
                                        <div class="ul-widget-app__browser-list-1 mb-2 "><i class="i-Spell-Check text-white teal-500 rounded-circle p-2 mr-3"></i><span class="text-15"><strong>Référence :</strong> {{ $fournisseur->liquide }} {{ $fournisseur->unite_liquide }}</span><span class="text-mute" style="display: none">2 April </span></div>
                                    @endif
                                    @if ($fournisseur->status)
                                        <div class="ul-widget-app__browser-list-1 mb-2 "><i class="i-Spell-Check text-white teal-500 rounded-circle p-2 mr-3"></i><span class="text-15"><strong>Statut :</strong> {{ $fournisseur->status == 1 ? 'Actif': 'Inactif' }}</span><span class="text-mute" style="display: none">2 April </span></div>
                                    @endif --}}
...
<section class="ul-product-detail__box">
    <div class="row">
        @php
            // $entrepots = $fournisseur->entrepots(Auth::user()->entreprise->id <> 1 ? Auth::user()->entreprise : null);
            $entrepots = $fournisseur->entrepots();
        @endphp
                ...
                <div class="card">
                    <div class="card-body">
                            ...
                            <div class="ul-widget-card__user-info row">
                                ...
                                <div class="col-md pt-3">
                                    <h3>
                                        ...
                                        <div>
                                            {{ number_format($fournisseur->stock_physique_entrepot($entrepot), 0, ',', ' ') }}
                                        </div>
                                    </h3>
                                    <h3>
                                        <div class="text-muted h5">Stock Virtuel</div>
                                        <div>
                                            {{ number_format($fournisseur->stock_virtuel_entrepot($entrepot), 0, ',', ' ') }}
                                        </div>
                                    </h3>
                                </div>
                            </div>
                        ...
                        <div class="pt-2 border-top px-md-2">
                            <a href="" class="btn btn-outline-success m-1" type="button" data-toggle="modal" data-target="#add_variation{{ $fournisseur->id }}{{ $entrepot->id }}entrée"><i class="nav-icon i-Down mr-1"></i> Entrée </a>
                            <a href="" class="btn btn-outline-danger m-1" type="button" data-toggle="modal" data-target="#add_variation{{ $fournisseur->id }}{{ $entrepot->id }}sortie"><i class="nav-icon i-Up mr-1"></i> Sortie </a>
                            <a href="{{ route('stock.stock_story_entrepot.index',['entrepot'=>$entrepot, 'product'=> $fournisseur]) }}" class="btn btn-outline-warning m-1"><i class="nav-icon i-Repeat-4 mr-1"></i> Historique </a>
                            <a href="" class="btn btn-outline-primary m-1 float-right" type="button" data-toggle="modal" data-target="#add_variation{{ $fournisseur->id }}{{ $entrepot->id }}transfert"><i class="nav-icon i-Data-Transfer mr-1"></i> Transfert </a>
